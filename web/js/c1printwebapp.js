...
$(document).ready(function() {
	// 検索画面の情報をこちらのjsに反映させる
	document.getElementById('address_from').value = window.opener.document.getElementById('address_from').value;
	document.getElementById('address_to').value = window.opener.document.getElementById('address_to').value;
	document.getElementById('drive').checked = window.opener.document.getElementById('drive').checked;
...
function initialize() {
	viewWidth = $("#print_map").css("width").replace("px", "");
	viewWidth *= 0.15;	// 並べられるようサイズ小さく
...
function showSteps(directionResult) {
	...
	// ストリートビューの初期化
	//document.getElementById("print_map").innerHTML = "";
	$('#print_map').empty();
...
function showStepsMoveView(fStep, tStep, index) {
	...
	//	document.getElementById("print_map").innerHTML += annotation + "<div id=\"street_view" + index + "\" class=\"street\">";
	document.getElementById("print_map").innerHTML += annotation + "<div id=\"street_view" + index + "\" style=\"width:" + viewWidth + "px; height:" + viewHeight + "px\"/>";
	if (tStep != null) {
		document.getElementById("print_map").innerHTML += "<img src='./image/a.jpg' class=\"direct\"/>";
	}
}
...
function createAnnotation(step, index, isLast) {
	...
	// 地点だけ返す
	return "<font size=\"+2\">" + index + " 地点:</font>";
}
...
function innerPointObj(pointObj) {
	...
	if (pointObjArray.length == destRoute.steps.length) {
		...
		for (var i = 0; i < length; i++) {
			...
			// 並べて表示するようスタイル部分削除
			var staticImage = "<img src=\"https://maps.googleapis.com/maps/api/streetview?size=" + viewWidth + "x" + viewHeight + "&location=" + latlng + "&heading=" + radian + "&pitch=0&fov=120&zoom=0\"/>";
			document.getElementById("print_map").innerHTML += annotation + staticImage;
			if (i + 1 < length) {
				document.getElementById("print_map").innerHTML += "<img src='./image/b.jpg' width=\"80\">";
			}
		}
	}
}
